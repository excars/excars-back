...
class ProfileRepository:
    ...
    async def save(self, profile: entities.Profile):
        await self.redis_cli.hmset_dict(self._get_key(profile.uid), **schemas.ProfileRedisSchema().dump(profile).data)
    ...
    async def expire(self, user_uid: str):
        await self.redis_cli.expire(self._get_key(user_uid), timeout=settings.USER_DATA_TTL_ON_CLOSE)

    async def persist(self, user_uid: str):
...
class RideRepository:
    ...
    @staticmethod
    def _get_remove_key(user_uid: str):
        return f"remove:ride:{user_uid}"
    ...
    async def update_request(self, ride_request: entities.RideRequest):
        ...
        await self.redis_cli.set(
            f"{ride_key}:passenger:{ride_request.passenger.uid}", value=status, expire=settings.RIDE_TTL
        )
    ...
    async def request_exists(self, ride_request: entities.RideRequest) -> bool:
        return bool(await self.redis_cli.exists(f"ride:{ride_request.ride_uid}:request:{ride_request.passenger.uid}"))
    ...
    async def delete_or_exclude(self, profile: entities.Profile):
        if profile.role == constants.Role.HITCHHIKER:
            await self._exclude(profile.uid)
        elif profile.role == constants.Role.DRIVER:
            await self._delete(profile.uid)

    async def _delete(self, ride_uid: str) -> None:
    ...
    async def _exclude(self, user_uid: str) -> None:
    ...
    async def expire(self, profile: entities.Profile):
        remove_key = self._get_remove_key(profile.uid)
        await self.redis_cli.set(remove_key, "true")
        await asyncio.sleep(settings.USER_DATA_TTL_ON_CLOSE)
        if await self.redis_cli.get(remove_key):
            await self.delete_or_exclude(profile)
            await self.redis_cli.delete(remove_key)

    async def persist(self, user_uid: str):
        await self.redis_cli.delete(self._get_remove_key(user_uid))
...
class StreamRepository:
    ...
    async def _produce(self, message_type: str, user_uid: str, payload: dict):
        await self.redis_cli.xadd(f"stream:{user_uid}", fields={"type": message_type, **payload})
    ...
    async def ride_updated(self, ride: entities.Ride):
        ...
        await self._broadcast(constants.MessageType.RIDE_UPDATED, user_uids, payload={"ride_uid": ride.uid})

    async def ride_cancelled(self, ride: entities.Ride):
        user_uids = [passenger.profile.uid for passenger in ride.passengers]

        await self._broadcast(constants.MessageType.RIDE_CANCELLED, user_uids, payload={"ride_uid": ride.uid})
...
class UserLocationRepository:
    ...
    async def save(self, user_uid: str, location: entities.UserLocation):
        await self.redis_cli.geoadd(
            self.KEY, latitude=location.latitude, longitude=location.longitude, member=str(user_uid)
        )
    ...
    async def list(self, user_uid: str) -> typing.List[entities.UserLocation]:
        ...
        geomembers = await self.redis_cli.georadiusbymember(self.KEY, member=user_uid, radius=50, unit="km")

        locations = await asyncio.gather(
            *[self.redis_cli.hgetall(f"user:{geomember.decode()}:location") for geomember in geomembers]
        )
    ...
    async def delete(self, user_uid: str):
        ...
        await self.redis_cli.delete(f"user:{user_uid}:location")
