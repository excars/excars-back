...
async def join(request, user):
    ...
    if errors:
        return sanic.response.json(errors, status=400)
    ...
    return sanic.response.json(schemas.ProfileSchema().dump(profile).data)
...
async def create_ride(request, user):
    ...
    ride_request = factories.make_ride_request(sender, receiver, status=constants.RideRequestStatus.REQUESTED)
    await repositories.RideRepository(redis_cli).create_request(ride_request)

    return sanic.response.json({"uid": ride_request.ride_uid})
...
async def update_ride(request, uid, user):
    ...
    if schema.errors:
        return sanic.response.json(schema.errors, status=400)
...
async def current_ride(request, user):
    ...
    return sanic.response.json(schemas.RideSchema().dump(ride).data)


@bp.route("/api/rides/leave", methods=["DELETE"])
@sanic_jwt.inject_user()
@sanic_jwt.protected()
async def leave(request, user):
    profile_repo = repositories.ProfileRepository(request.app.redis)
    ride_repo = repositories.RideRepository(request.app.redis)
